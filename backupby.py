...
def on_add_data():
    fitness_time = float(entry_fitness.get())
    sleep_time = float(entry_sleep.get())
    health_issue = entry_health_issue.get()
    new_data = {"Time spent on fitness": [fitness_time], "Time spent on sleep": [sleep_time], "Health issue": [health_issue]}
...
label_health_issue = tk.Label(frame_input, text="Health issue:")
label_health_issue.pack(side=tk.LEFT)
entry_health_issue = tk.Entry(frame_input)
entry_health_issue.pack(side=tk.LEFT)
...
# add_data_button = tk.Button(frame_buttons, text="Add Data", command=on_add_data)
